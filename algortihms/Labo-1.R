...
#1.1 Filter out items
item_freq <- itemFrequency(Groceries)
support_threshold <- 0.03
Groceries_filtered <- Groceries[, item_freq >= support_threshold]
itemFrequencyPlot(Groceries_filtered, support = support_threshold, cex.names = 0.5)

#1.2 Group some items
...
itemFrequencyPlot(Groceries_filtered2, support=0.03, cex.names = 0.5)

#1.3 Remove NAs
Groceries_df <- as(Groceries, "data.frame")
transactions_with_NAs <- rowSums(is.na(Groceries_df)) > 0
Groceries_no_NAs <- Groceries[!transactions_with_NAs]
itemFrequencyPlot(Groceries_no_NAs, support=0.03, cex.names = 0.5)


#2 Changing support and confidence
rules <- apriori(Groceries, parameter = list(support = 0.03, confidence = 0.3))
summary(rules)
inspect(rules)

#3 Focusing on specific RHSs
rules_ham <- subset(rules, subset = rhs %in% "ham")

#4 Limiting the size of the LHS
rules2 <- apriori(Groceries, parameter = list(support = 0.03, confidence = 0.3, maxlen = 3))
summary(rules2)
inspect(rules2)
...
rules_sorted <- sort(rules_sorted, by = "lift", decreasing = TRUE)

# Remove redundant rules
rules_no_redundancy <- rules_sorted[!duplicated(subset(rules_sorted, select = c("lhs", "rhs")))]
