...
# Data Analytics Laboratory: Neural Networks (I)
##########################################################

library(MASS)
library(nnet)
library(doParallel) # Parallelism
set.seed (1233)
...
# This will build a multinomial regression (generalizes logistic regression for
...
## The basic parameters are 'size' and 'decay' (the regularization constant =
## lambda) R typically detects the kind of task and adjust the linout, entropy,
## softmax parameters (?) In this case it builds a MLP with then output neurons,
## with the softmax function and uses the generalized cross-entropy as error
## function to be minimized

## In general it is a good idea to start by scaling the inputs; this is
## important to avoid network 'stagnation' (premature convergence); in this case
## it brings trouble because of the data distribution (e.g., there are columns
## of '0's that should be removed, but this is not clear)
...
## In particular, understand why the total number of weights is 160, what
## 'initial  value' and 'final  value' are and what does 'converged' mean
...
## i1,i2,i3,... are the inputs, h1, h2, ... are the hidden neuron's outputs, b
## is the bias (offset)

## As you can see, some weights are large (two orders of magnitude larger then
## others) This is no good, since it makes the model unstable (i.e., small
## changes in some inputs may entail significant changes in the network, because
...
## We get around a 7.5-9%, so it seems that this MLP is overfitted; we need to
## work harder ...
...
## We are going to do the modelling in a principled way now. Using repeated CV
## to select the best combination of 'size' and 'decay'

## Just by curiosity, let me show you that we can fit almost any dataset (in the
## sense of reducing the training error):
...
## {caret} is an excellent package for training control, once you know what all
## these concepts are

## WARNING: if the package is not installed in your computer, installation needs
## some previous packages
library(caret)

## For a specific model, in our case the neural network, the function train() in
## {caret} uses a "grid" of model parameters and trains the network using a
## given resampling method (in our case we will be using 10x10 CV).  All
## combinations are evaluated, and the best one (according to 10x10 CV) is
## chosen and used to construct a final model, which is then refit using the
## whole training set

## Thus train() returns the constructed model (exactly as a direct call to
## nnet() would)

## In order to find the best network architecture, fix a large number of hidden
## units in one hidden layer, and explore different regularization values
## (recommended)

## doing both (explore different numbers of hidden units AND regularization
## values) is usually a waste of computing resources (but notice that train()
## would admit it)
...
## Therefore, we are going to do some parallelism, as each parameter combination
## can run in parallel perfectly.
## Warning2: Change this number to the number of CPU threads you want to use!
n_threads <- 4
...
stopCluster(cl) # Empty the cluster sessions and free memory

### A more realistic experimental setting (with more computational power, but
### only if really needed) would require a larger size and set of decays (maybe
### by refinement)
...
## Recover the model so that we can resume the code here instead of training
## again
load("5x2-CV.mod")
####################################
...
## We get 5.5% after all this work; it seems that the use of a more
## sophisticated learner does is not justified (or is it? what do you think?)
